...
# update our fog
func update_fog(pos):
	var fogi = Image.create(fogWidth, fogHeight, false, Image.FORMAT_RGBA8)
	fogi.fill(Color(0,0,0, 1))
	fogi.blend_rect(lightImage, light_rect, pos - light_offset)
	fogTexture.update(fogi)
...
func _process(delta):
	...
	if (time_since_last_fog_update >= debounce_time):
		var player_input = Player.velocity.length()
		if player_input > 7000:
